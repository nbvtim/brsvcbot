...
// -c camera-id Идентификатор используемой камеры (см. termux-camera-info), по умолчанию: 0
        // const termux_camera_info = spawn("termux-camera-info")
...
// termux-brightness [0 - 255] - яркость
        // const termux_brightness_0 = spawn("termux-brightness",[0])
        // termux_brightness_0.on("close", data=>{
        //     bot.sendMessage(id, `яркость: 0`)
        // })
        // const termux_brightness_255 = spawn("termux-brightness",["120"])
...
// -o смещение смещения в списке журнала вызовов (по умолчанию: 0)
        // const termux_call_log = spawn("termux-call-log")
...
// cat file.txt | termux-clipboard-set
        // const termux_clipboard_set = spawn("termux-clipboard-set", ["jsldjflsdfhsdfhklskdjf"])
        // termux_clipboard_set.on("close", data=>{
        //     bot.sendMessage(id, "Записано в буфер")
        // })
        // const termux_clipboard_get = spawn("termux-clipboard-get")
        // termux_clipboard_get.stdout.on("data", data=>{
        //     bot.sendMessage(id, `Содержимое буфера:\n${data}`)
        // })

// termux-contact-list
        // const termux_contact_list = spawn("termux-contact-list")
        // let fulldata = ""
        // let counter = 0
...
//     [-t title] установить заголовок диалога (необязательно)
        // const termux_dialog = spawn("termux-dialog", ["-l"])
...
// -p путь путь для сохранения загруженного файла
    // const termux_download = spawn("termux-download", ["-d описание уведомления", "-t заголовок уведомления", "-p ./", "https://web.telegram.org/k/#@lydi_sochi"])
