...
async function brsvcbot(msg){
    ...
    if(security(msg.chat.id)){
        ...
        /*временная*/if(obj[msg.chat.id] !== "/auto" && obj[msg.chat.id] != "/key"){bot.sendMessage(msg.chat.id,`Вод не поддерживается\nперейдите в раздел /auto или /key`)}
...
function security(id){
    for(i in getData()){
        if(getData()[i].name === "users"){
            getId = +getData()[i].data.join("\n").match(RegExp(id,"g"))
        }
    }
    if(getId === id){return true}
    else{bot.sendMessage(id, "<b>Нет доступа !!!\nВы можете прислать данные в формате</b>\nФИО:\nНомер телефона:\nДата рождения:\n<b>После одобрения Вам предоставят доступ</b>", {parse_mode:"HTML"});return false}
}